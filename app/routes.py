...
from app.forms import LoginForm, CreateAccountForm, SearchIngredientsForm, AddIngredientsForm, DeleteIngredientsForm, \
    SearchRecipesForm, EditIngredientForm, EditAllergiesForm, ChangePasswordForm, CommentForm, timeLimitForm
from flask_login import current_user, login_user, logout_user, login_required
from app.models import User, Ingredient, Allergy, Recipe, RecipeIngredient, Comment
from werkzeug.urls import url_parse
from app.api import query_ingredients, query_recipes, query_recipe_information, query_recipes_2
...
def view_recipe(id):
    ...
    if recipe is None:
        ...
        recipe = Recipe(api_id=id, name=recipe_information['title'], image=recipe_information['image'],
                        instructions=recipe_information['instructions'],
                        preparation_time=int(recipe_information['preparationMinutes']))
        db.session.add(recipe)
        for extendedIngredient in recipe_information['extendedIngredients']:
            ingredient = RecipeIngredient(name=extendedIngredient['name'], amount=float(extendedIngredient['amount']),
                                          unit=extendedIngredient['unit'], image=extendedIngredient['image'],
                                          recipe=recipe)
...
def pantry():
    ...
    ingredients = current_user.ingredients.filter(Ingredient.expiration_date != None).order_by(
        Ingredient.expiration_date).all()
...
def delete_pantry_items():
    ...
    form.ingredients.choices = [(ingredient.id,
                                 f"{ingredient.name},{ingredient.expiration_date.strftime('%Y-%m-%d') if ingredient.expiration_date else ''},{ingredient.image}")
                                for ingredient in ingredients]
